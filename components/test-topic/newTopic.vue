	...
	<view class="container" @touchstart="touchStart" @touchmove="touchMove" @touchend="touchEnd">
		<view class="serial">
			<view class="headlines">
				第{{qid + 1}}题
				<text v-if="topics[qid].type === 1">(单选)</text>
				<text v-else-if="topics[qid].type === 2">(多选)</text>
				<text v-else-if="topics[qid].type === 3">(判断)</text>
				<text v-else-if="topics[qid].type === 4">(填空)</text>
				<text v-else-if="topics[qid].type === 5">(问答)</text>
			</view>
			<view class="score" v-if="isType==='simulation'">({{topics[qid].score}}分)</view>
			<view class="count">({{(qid + 1) + '/' + topics.length}})</view>
		</view>
		<view class="title">{{topics[qid].title}}</view>
		<!-- 单选 -->
		<vanRadioGroup @change.native="onChoose" class="options" v-if="topics[qid].type === 1">
		...
		<!-- 多选 -->
		<vanCheckboxGroup class="options" :value="topics[qid]['myAnswer']" @change.native="onSelect" v-else-if="topics[qid].type === 2">
			<view class="cell" v-for="item in options" :key="item.id">
				<van-checkbox use-icon-slot :name="item.name" class="clearfix">
					<icon :class="['iconfont',(topics[qid]['myAnswer'].indexOf(item.name) > -1) ? item.actIcon : item.icon]"></icon>
					<view :class="['text',(topics[qid]['myAnswer'].indexOf(item.name) > -1) ? 'textActive' : ' ']">{{topics[qid][item.value]}}</view>
				</van-checkbox>
			</view>
		</vanCheckboxGroup>
		<!-- 判断 -->
		<vanRadioGroup @change.native="onChoose" class="options" v-else-if="topics[qid].type === 3">
			<view class="cell">
				<vanRadio use-icon-slot name="A" class="clearfix">
					<icon :class="['iconfont',(topics[qid]['myAnswer'] === 'A') ? 'my-icon-activeA' : 'my-icon-A']"></icon>
					<view :class="['text',topics[qid]['myAnswer'] === 'A' ? 'textActive' : ' ']">{{topics[qid]['optionA']}}</view>
				</vanRadio>
			</view>
			<view class="cell">
		...
		<!-- 填空 -->
		<vanField
			v-else-if="topics[qid].type === 4"
		    :value="topics[qid]['myAnswer']"
			label="回答"
		    placeholder="请输入答案"
	...
	export default {
		...
		methods:{
					...
					if(this.isType === 'test'){
						...
						if(res.data.code === 0){
							...
							for(let i = 0; i < this.topics.length; i++){
								console.log('循环了',i,this.qid)
								if(this.topics[i].id === this.topics[this.qid].id){
									// 如果数组仅剩一项数据
		    ...
		    onChoose(event){
			    ...
			    console.log('单选',this.topics,this.qid);
		    },
			//多选
			onSelect(event){
			    console.log('多选',event.detail);
			    this.$set(this.topics[this.qid],'myAnswer',event.detail);
			    console.log('多选',this.topics,this.qid);
			},
			//填空
			inBlank(event){
